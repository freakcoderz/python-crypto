...
# region User
        def make_user(self, snowflake, address):
            ...
            to_exec = "INSERT INTO users (snowflake_pk, balance, balance_unconfirmed, allow_soak, address) VALUES(%s, %s, %s, %s, %s)"
            cursor.execute(
                to_exec, (str(snowflake), '0', '0', '1', str(address)))
            cursor.close()
        ...
        def check_for_user(self, snowflake):
            ...
            cursor = self.__setup_cursor(
                pymysql.cursors.DictCursor)
            to_exec = "SELECT snowflake_pk, address, balance, balance_unconfirmed, allow_soak, last_msg_time, rain_last_msg_time, rain_msg_count FROM users WHERE snowflake_pk LIKE %s"
        ...
        def get_user(self, snowflake):
            cursor = self.__setup_cursor(
                pymysql.cursors.DictCursor)
            to_exec = "SELECT snowflake_pk, balance, balance_unconfirmed, address, allow_soak, last_msg_time, rain_last_msg_time, rain_msg_count FROM users WHERE snowflake_pk LIKE %s"
            cursor.execute(to_exec, (str(snowflake)))
            result_set = cursor.fetchone()
            cursor.close()
            return result_set
        ...
        def get_user_by_address(self, address):
            cursor = self.__setup_cursor(
                pymysql.cursors.DictCursor)           
            to_exec = "SELECT snowflake_pk, balance, balance_unconfirmed, address, allow_soak, last_msg_time, rain_last_msg_time, rain_msg_count FROM users WHERE address LIKE %s"
