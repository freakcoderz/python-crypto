...
class Balance:
    ...
    async def do_embed(self, name, server, db_bal, db_bal_unconfirmed, stake_total, donate_total):
        ...
        embed=discord.Embed(title="**⚠WARNING Support is ending, withdraw funds!!⚠**", color=self.embed_color)
        ...
        try:
            ...
            if server is not None:
                await self.bot.say("{}, I PMed you your **Balance**! Make sure to double check that it is from me! Support is ending, withdraw funds!!".format(name.mention))
    ...
    async def userbal(self, ctx, *args: discord.Member):
        """Displays a members balance (ADMIN ONLY)"""
        # Set important variables
        ...
        for user in users:
            ...
            #mysql.check_for_user(snowflake)
            if mysql.check_for_user(snowflake) is None:
                await self.bot.say("{} **:warning:Could not find this member:warning:**".format(ctx.message.author.mention, user.mention))
                return
