...
class Stake():
    ...
    async def do_stake_embed(self, name, db_bal, db_bal_unconfirmed, stake_total_amt, stake_total_received, reg_users_eligible):
        ...
        embed=discord.Embed(title="**Aevo bot stake payouts**", color=self.embed_color)
        embed.add_field(name="Stakes paid out", value="{:.8f} {}".format(round(float(db_bal), 8),self.currency_symbol))
        ...
        embed.add_field(name="Total Stakes since birth", value="{}".format(int(stake_total_amt)))
    ...
    async def stake(self, ctx):
        ...
        for users in all_regusr:
            ...
            if (float(user_bal) >= 100.00000000):
